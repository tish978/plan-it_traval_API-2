...
MONGODB_URI = os.getenv("MONGODB_URI", "mongodb+srv://satishbisa:hellohello123@cluster0.b7fpk09.mongodb.net/?retryWrites=true&w=majority&ssl=true")
...
def add_cache_control(response: Response, max_age: int):
    response.headers["Cache-Control"] = f"publix, mac-age={max_age}"

# Endpoint used for sending emails on Contact Page
@app.post("/send_email")
def send_email( response: Response, request: Request, name: str = Form(...), email: str = Form(...), message: str = Form(...)):
    add_cache_control(response, 60)
    # Specify details for the email
    message = Mail(
        from_email=email,
        to_emails='satish.bisa@gmail.com',
...
# Endpoint to query the database based on the end user's Plan Trip Survey answers
@app.post("/query", response_class=HTMLResponse)
def query_destinations(request: Request, response: Response,q1: list = Form(...), q2: list = Form(...), q3: list = Form(...),
                             q4: list = Form(...), q5: list = Form(...), q6: list = Form(...), q7: list = Form(...),
                             q8: list = Form(...), q9: list = Form(...)):
    add_cache_control(response, 300)  # Cache for 5 minutes
    collection = database['destinations']
    reports_collection = database['reports']
...
# Endpoint for Generate Report Functionality
@app.post("/make-report")
def make_report(request: Request, response: Response):
    add_cache_control(response, 300)  # Cache for 5 minutes
    reports_collection = database['reports']
    destination_collection = database['destinations']
...
def home(response: Response):
    add_cache_control(response, 3600)  # Cache for 1 hour
    return FileResponse("templates/home.html")
...
# Endpoint to perform the Sign Up functionality
@app.post("/sign-up")
def handle_sign_up(request: Request, username: str = Form(...), password: str = Form(...)):
    # Check if username already exists
    collection = database['users']
    existing_user = collection.find_one({"username": username})
    if existing_user:
...
@app.post("/sign-up")
async def handle_sign_up(response: Response, request: Request, username: str = Form(...), password: str = Form(...)):
    add_cache_control(response, 60)  # Cache for 1 minute
    # Check if username already exists
    ...
    existing_user = await collection.find_one({"username": username, "password": password})
